...
ok(/if \(quietForReload\(\)\) \{ reloadForUpdate\(\); return; \}/.test(html), 'a quiet board reloads straight away')
ok(/function reloadForUpdate\(\) \{\s*if \(IN_APP\) \{ try \{ APP_BRIDGE\.reloadConsole\(\); return; \} catch \{\} \}\s*location\.reload\(\);/.test(html),
  'in the tablet app through PointHubApp.reloadConsole(), anywhere else an ordinary reload')
